<?php
    ...
    $duinoPseudo = mysqli_real_escape_string($conn, $_POST['duinoPseudo']);
    if(!empty($full_url) && filter_var($full_url, FILTER_VALIDATE_URL)){
        ...
        if(mysqli_num_rows($sql) > 0){
            ...
            $sql2 = mysqli_query($conn, "INSERT INTO url (full_url, shorten_url, clicks, duinoPseudo) 
                                         VALUES ('{$full_url}', '{$ran_url}', '0', '{$duinoPseudo}')");
            if($sql2){
